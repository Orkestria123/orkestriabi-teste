...
BEGIN
  ...
  -- Quem está alocado FORA do grupo hoje. É a única coisa que a
  -- conferência precisa, e sai só do de-para + plano — sem comparar
  -- nome nenhum. Antes disto vinha depois do cálculo de semelhança
  -- para TODAS as contas do ECD contra TODO o plano, que é o que
  -- estourava o tempo limite no modo "Conferir grupos".
  CREATE TEMP TABLE _fora_base ON COMMIT DROP AS
    SELECT d.conta_codigo, g.descricao AS nome, d.conta_padrao_codigo AS hoje,
     ...
     LIMIT CASE WHEN _so_conferir THEN 500 ELSE NULL END;
  ...
  -- O ECD com nome já normalizado. No modo conferência, só as contas
  -- que aparecem na lista de fora do grupo — no máximo 500.
  ...
  CREATE TEMP TABLE _cand ON COMMIT DROP AS
    SELECT o.codigo AS ecd_codigo, f.codigo AS plano_codigo,
           (o.norm = f.norm) AS nome_igual,
           round(2.0 * (
             SELECT count(*) FROM unnest(o.palavras) w
              WHERE f.palavras @> ARRAY[w]
           ) / NULLIF(
             COALESCE(array_length(o.palavras, 1), 0)
           + COALESCE(array_length(f.palavras, 1), 0), 0), 4) AS nota
      FROM _origem o
      JOIN _folha f
        ON (f.classificacao = o.grupo_classificacao
            OR left(f.classificacao, length(o.grupo_classificacao) + 1)
               = o.grupo_classificacao || '.')
       AND (o.tipo_alvo IS NULL OR f.tipo = o.tipo_alvo);
  ANALYZE _cand;
