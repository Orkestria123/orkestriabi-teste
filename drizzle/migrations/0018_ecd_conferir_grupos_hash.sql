...
BEGIN
  ...
  -- Quem está alocado FORA do grupo hoje. Sai só do de-para + plano.
  ...
  --
  -- A semelhança sai de um join por PALAVRA, não de um subselect por
  -- par de contas. O desenho antigo comparava cada conta do ECD com
  -- cada folha do grupo e, para cada par, contava palavra por palavra —
  -- centenas de milhares de contagens, o que estourava o tempo limite
  -- do banco na conferência. Aqui as palavras entram no join uma vez.
