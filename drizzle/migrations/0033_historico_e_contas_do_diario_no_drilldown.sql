-- 1) Histórico do ECD: string vazia não é histórico. Volta a ser NULL,
--    para a tela poder dizer "Sem histórico no arquivo" em vez de célula
--    em branco silenciosa.
UPDATE public.lancamentos_diario
   SET historico = NULL
 WHERE historico IS NOT NULL AND btrim(historico) = '';

-- 2) `drilldown_contas`: a otimização "se existe saldo mensal no período,
--    ignore o diário" olhava a empresa inteira, não a conta — bastava UMA
--    conta com saldo mensal no mês para que contas que só têm partida no
--    diário desaparecessem da gaveta. Agora o diário entra sempre, restrito
--    ao período (índice por company_id + competencia).
