...
export const createClientUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: isTenantAdmin } = await supabaseAdmin
      ...
      .maybeSingle();
    const mesmoEscritorio = callerProfile?.tenant_id === company.tenant_id;
    if (!isOrk && (!isTenantAdmin || !mesmoEscritorio)) {
...
/**
 * Liga um usuário já existente à empresa (mesmo escritório da Casa do Vidro, etc.).
 * Quem foi criado como "novo tenant" fica num escritório vazio e não vê os dados.
 */
export const vincularUsuarioEmpresa = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) =>
    z
      .object({
        ...
        company_id: z.string().uuid(),
        como: z.enum(["client", "tenant_admin"]).default("client"),
      })
      .parse(input),
  )
  .handler(async ({ context, data }) => {
    ...
    const companyId = data.como === "client" ? data.company_id : null;
    const { error: pErr } = await supabaseAdmin
      .from("profiles")
      .update({ tenant_id: company.tenant_id, company_id: companyId })
      .eq("id", data.user_id);
    if (pErr) throw new Error(pErr.message);
