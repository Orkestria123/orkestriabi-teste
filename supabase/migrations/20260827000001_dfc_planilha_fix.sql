-- ============================================================
-- AJUSTE 19 — reaplica as funções da planilha da DFC
-- ============================================================
--
-- Sintoma: "Could not find the function public.dfc_exportar(...) in the
-- schema cache" ao exportar as alocações.
--
-- A função em si está certa — os nomes dos parâmetros batem com o que o
-- PostgREST procura. O que falha é ela CHEGAR no banco:
--
--   1. o arquivo do ajuste 17 tem o MESMO nome do ajuste 16
--      (20260826000001). O Supabase controla migration por nome de
--      arquivo: se aquele nome já consta em supabase_migrations, um
--      `migration up` / `db push` PULA o arquivo, mesmo com conteúdo
--      novo. Só `db reset` reexecuta tudo. Por isso este arquivo tem
--      data nova — assim ele roda de qualquer jeito.
--
--   2. o PostgREST guarda um cache do schema. Criar a função com o
--      serviço no ar não basta; ele precisa ser avisado. É o NOTIFY no
--      fim do arquivo.
--
-- Reaplicável quantas vezes quiser: tudo é DROP IF EXISTS + CREATE.
-- Não altera nada além das funções da DFC.
...
-- ------------------------------------------------------------
-- 5) Avisa o PostgREST para recarregar o schema
--
-- Sem isto a função existe no banco e a API continua dizendo que não
-- encontrou — que é exatamente o erro relatado.
-- ------------------------------------------------------------
