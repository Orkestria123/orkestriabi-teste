-- Custo vs despesa no plano, além de fixo/variável.
-- Contas já marcadas fixo/variável recebem a classe pelo grupo do plano.

ALTER TABLE public.plano_contas
  ADD COLUMN IF NOT EXISTS classe_gasto text;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'plano_contas_classe_gasto_chk'
  ) THEN
    ALTER TABLE public.plano_contas
      ADD CONSTRAINT plano_contas_classe_gasto_chk
      CHECK (classe_gasto IS NULL OR classe_gasto IN ('custo','despesa'));
  END IF;
END $$;

COMMENT ON COLUMN public.plano_contas.classe_gasto IS
  'custo | despesa — distingue CPV/CMV/CSP de despesas operacionais. Filho herda o ancestral mais específico, igual a tipo_custo.';

UPDATE public.plano_contas
   SET classe_gasto = 'custo'
 WHERE tipo_custo IS NOT NULL
   AND classe_gasto IS NULL
   AND (
        classificacao = '3.02' OR classificacao LIKE '3.02.%'
     OR classificacao = '3.03' OR classificacao LIKE '3.03.%'
     OR classificacao = '3.04' OR classificacao LIKE '3.04.%'
     OR classificacao = '3.05.01' OR classificacao LIKE '3.05.01.%'
     OR classificacao = '3.05.98' OR classificacao LIKE '3.05.98.%'
   );

UPDATE public.plano_contas
   SET classe_gasto = 'despesa'
 WHERE tipo_custo IS NOT NULL
   AND classe_gasto IS NULL
   AND (
        classificacao = '3.06' OR classificacao LIKE '3.06.%'
     OR classificacao = '3.15' OR classificacao LIKE '3.15.%'
     OR classificacao = '3.17' OR classificacao LIKE '3.17.%'
     OR classificacao = '3.18' OR classificacao LIKE '3.18.%'
     OR classificacao = '3.19' OR classificacao LIKE '3.19.%'
   );
