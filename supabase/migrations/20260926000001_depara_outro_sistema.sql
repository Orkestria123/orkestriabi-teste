-- De-para de "Outro sistema": a fila e a tradução passam a partir do
-- MOVIMENTO (diário/saldos), não só do plano da empresa. Sem isso a
-- tela ficava vazia — o ERP não precisa ter um plano importado à parte.
-- Sugestão só por classificação (rápida). Carregar contas do arquivo
-- do sistema alimenta classificação/nome para o lote.
...
-- Tradução também sem exigir plano próprio: conta com saldo/lançamento
-- entra no mapa. Senão o de-para gravava e a DRE continuava zerada.
